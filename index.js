...
const getPokemnon = async() => {
  const pokemonData = await fetch('https://pokeapi.co/api/v2/pokemon?limit=10')
  const pokemonJson = await pokemonData.json()

  let name;
  let img;
  const pokemons = await pokemonJson.results.map(async(poke) => {
    const pokeData = await fetch(poke.url)
    const pokeJson = await pokeData.json()

    return {
      name: poke.name,
      img: pokeJson.sprites.other['official-artwork'].front_shiny,
    }
  })

  pokemons.forEach(async(poke) => {
    const pokeObj =  await poke
    
    renderPokemon(pokeObj)
  })
};
